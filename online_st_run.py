...
def run_teacher_pretraining(n_teacher_samples: str):
    ...
    train_teacher_cmd = f"python run.py --experiment_id=multi_ds_vessel --sam_model=vit_b --learning_rate=0.0003 --prompt_encoder_lr=0.0001 --image_encoder_lr=0.00001 --mask_decoder_lr=0.0001 --batch_size=3 --epochs=1 --weight_decay=1e-4 --early_stopping_patience=10 --visualize_n_segmentations=5 --gamma=0.95 --step_size=3 --best_model_metric=IoU --minimize_best_model_metric=false --prompt_encoder_checkpoint={teacher_checkpoint_dir}/prompt_encoder.pt --amp=true --results_subdir_name={teacher_experiment_subdir_name}"
...
def run_student_st(teacher_checkpoint: str, epochs: int, learning_rate: float):
    ...
    exec(
        f"python run.py --experiment_id=self_learning_experiment --use_wandb=false --wandb_tags={shlex.quote(tags)} --wandb_experiment_name=vessels_gt_oneline_st_{n_teacher_samples}_samples --limit=5000 --sam_model=vit_b --learning_rate={str(learning_rate)} --batch_size=20 --epochs={str(epochs)} --weight_decay=1e-4 --gamma=0.9 --step_size=2 --best_model_metric=IoU --minimize_best_model_metric=false --drive_test_equals_val=false --amp=true --augment_train=true --ema_decay=0.95 --from_checkpoint={teacher_checkpoint} --results_subdir_name={student_experiment_subdir_name} --return_best_model=false"
    )

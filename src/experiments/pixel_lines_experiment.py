...
from src.datasets.refuge_dataset import RefugeDataset
from src.datasets.pixel_lines_dataset import PixelLinesDataset, PixelLinesDatasetArgs
...
class PixelLinesExperimentArgs(
    BaseExperimentArgs, AdamArgs, StepLRArgs, PixelLinesDatasetArgs, AutoSamModelArgs
):
...
class PixelLinesExperiment(BaseExperiment):
    def __init__(self, config: dict[str, Any], yaml_config: YamlConfigModel):
        self.config = PixelLinesExperimentArgs(**config)
        self.retina_data = PixelLinesDataset(config=self.config, yaml_config=yaml_config)
        super().__init__(config, yaml_config)

    def get_name(self) -> str:
        return "pixel_lines_experiment"
    ...
    def get_args_model(cls):
        return PixelLinesExperimentArgs
    ...
    def run_after_training(self, trained_model: BaseModel):
        ...
        def predict_visualize(split: Literal["train", "test"]):
            ...
            ds = cast(PixelLinesDataset, self._create_dataset(split))
            print(
            ...
            for i in range(min(len(ds.samples), self.config.visualize_n_segmentations)):
                ...
                img, mask = model.segment_image(np.array(sample.img), self.yaml_config.fundus_pixel_mean, self.yaml_config.fundus_pixel_std, self.yaml_config.fundus_resize_img_size)
